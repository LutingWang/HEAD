...
model = dict(
    ...
    distiller=dict(
        ...
        adapts={
            ...
            dict(
                type='Index',
                fields=['fcos_cls_reshaped', 'bbox_poses'],
            ),
            'fcos_cls_decoupled':
            dict(
                type='Linear',
                fields=['fcos_cls_indexed'],
                in_features=256,
                out_features=1024,
            ),
        },
        losses=dict(
            loss_self_retina=dict(
                type='MSELoss',
                fields=['fcos_cls_adapted', 'retina_cls_detached'],
                parallel=True,
                weight=1.0,
            ),
            loss_self_rcnn=dict(
                type='MSELoss',
                fields=['fcos_cls_decoupled', 'rcnn_bbox_filtered'],
                weight=1.3,
            ),
        ),
        schedulers=dict(
            warmup=dict(
                type='WarmupScheduler',
                fields=['loss_self_retina', 'loss_self_rcnn'],
                iter_=2000,
            ),
            early_stop=dict(
